...
| `SGLANG_LOG_MOE_RUNTIME`                  | Enable sampled MoE runtime logs for runner backend, A2A backend, fused path, shapes, quantization mode, kernel config, and expert routing distribution. | `false`                      |
| `SGLANG_LOG_MOE_RUNTIME_INTERVAL`         | Log every Nth MoE runtime event per event/layer/backend key when `SGLANG_LOG_MOE_RUNTIME` is enabled. The first event is always logged. | `64`                         |
| `SGLANG_LOG_MOE_RUNTIME_RANKS`            | Comma-separated global ranks allowed to emit MoE runtime logs. Empty means all ranks.                                            | `""`                         |
| `SGLANG_LOG_MOE_RUNTIME_TOP_EXPERTS`      | Number of hottest experts included in MoE runtime routing summaries.                                                             | `8`                          |
